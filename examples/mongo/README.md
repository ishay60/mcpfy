...
A minimal mcpfy config that exposes a single MongoDB database to Claude Desktop, Cursor, or Claude Code over stdio.
...
The Mongo driver (`mongodb`) is pulled in as an optional dep of `@mcpfy/connector-mongo`.
...
5. Results are redacted, size-capped, and wrapped in an `<mcpfy-data>` "untrusted-data" block before reaching the model.
